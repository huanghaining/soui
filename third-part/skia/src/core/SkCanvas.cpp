...
void SkCanvas::onDrawText(const void* text, size_t byteLength, SkScalar x, SkScalar y,
    ...
    while (iter.next()) {
		...
		if (paint.m_emojiBmp.width() && paint.m_emojiBmp.height())
		{
			iter.fDevice->drawText(iter, text, byteLength, x, y, dfp.paint(),&(paint.m_emojiBmp));
		} 
		else
		{
...
void SkCanvas::drawText(const void* text, size_t byteLength, SkScalar x, SkScalar y,
	...
	//this->drawBitmap(m_emojiBmp, x, y, &paint);
}
