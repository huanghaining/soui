...
string dealString(const wchar_t *content, const string &emoji)
{
	wstring str = wstring(content);
	int pos = 0;
	string strAim = "";
	bool hasEmoji = false;
	string curEmoji = "";
	...
	for (int j = 0; j < iTotalLength; j++)
	{
		if (!hasEmoji)
		{
			...
			{
				//strAim.append(s2ws(str.substr(pos, 1)));
			}
		}
		else
		{
...
bool IsEmoji(const wchar_t wstrText[], int length)
{
	SkTDArray<wchar_t> tmp;
	tmp.setCount(length);
	memcpy(tmp.begin(), wstrText, length*sizeof(wchar_t));
	...
	for (int nIndex = 0; nIndex < tmp.count(); ++nIndex)
	{
...
//#include "../../SOUI/include/interface/render-i.h"
...
void drawEmoji()
{
	...
	//IBitmap bitmap;
	////SkImageDecoder::DecodeFile("C:\\Users\\Administrator\\Desktop\\123.jpg", &bitmap);
	//
	//bitmap.LoadFromFile(("C:\\Users\\Administrator\\Desktop\\123.jpg"));

	//if (IsEmoji(strW, cchLen))
	//{
	//	// 			IBitmap * pBmp = SResLoadFromFile::LoadImage(_T("C:\\Users\\Administrator\\Desktop\\123.jpg")); 
	//	// 			CRect rcClient = GetClientRect();
	//	// 			CSize szBmp(pBmp->Size());
	//	//DrawBitmap(rcClient, pBmp, 0, 0)
	//	bitmap->LoadFromFile(L"E:\\soui-master\\soui-master\\demo\\uires\\image\\apng_haha.png");
	//	LPRECT pNRec = pRc;
	//	SIZE s = bitmap->Size();
	//	pNRec->left = pNRec->right + 100;
	//	pNRec->bottom = pNRec->top + 100;
	//	DrawRectangle(pNRec);
	//	DrawBitmap(pNRec, bitmap, pNRec->left, pNRec->top);
	//	return S_OK;
	//}
}
...
//#include <cstring>
//#include <cstddef>
//SkBitmap* loadImageFromResource( char* fileName)
//{
//	//TCHAR szLog[512] = { 0 };
...
//	size_t size = 0;
//
//	//KCstd::cout << "Attempting to open " << INPUT_FILENAME << std::endl;
//
...
//	data[size] = '\0';
//
//	SkMemoryStream memoryStream(data, size);
...
//	if (decoder->decode(&memoryStream,
...
void SkDraw::drawText(const char text[], size_t byteLength,
	SkScalar x, SkScalar y, const SkPaint& paint,const SkBitmap* bitmap) const {
	...
	while (text < stop) {
		...
		int ecount;
		wstring str = wstring(wtext);
		if (isEmoji(str.at(0), ecount))
		{
			wtext = wtext + ecount;
			SkPaint paint1(paint);
			SkScalar matrixScale = paint1.setupForAsPaths();
			SkRect src = SkRect::MakeLTRB(0, 0, bitmap->width(), bitmap->height());
			
			SkRect dst = SkRect::MakeLTRB(SkFixedFloorToInt(fx), SkFixedFloorToInt(fy) - bitmap->height(), SkFixedFloorToInt(fx) + bitmap->width(), SkFixedFloorToInt(fy) + bitmap->height());
			fDevice->drawBitmapRect(*this, *bitmap, &src, dst, paint, SkCanvas::DrawBitmapRectFlags::kBleed_DrawBitmapRectFlag);
			fx += SkIntToFixed(bitmap->width());
			//fy += SkIntToFixed(bitmap->height());
			/*SkMatrix matrix;
			matrix.setTranslate(fx, fy);
			matrix.setScale(matrixScale, matrixScale);
			this->drawBitmap(*bitmap, matrix, paint);*/
		}
		text = (char*)wtext;
		if (text >= stop)
		{
...
void SkDraw::drawText(const char text[], size_t byteLength,
    ...
    while (text < stop) {
		...
		if (isEmoji(str.at(0), ecount))
		{
			...
			matrix.setTranslate(fx, fy);
			matrix.setScale(matrixScale, matrixScale);
			this->drawBitmap(paint.m_emojiBmp, matrix, paint);
		}
		text = (char*)wtext;
		if (text >= stop)
		{
        ...
        fx += autokern.adjust(glyph);
		//SkBitmap *bitmap = loadImageFromResource("C:\\Users\\Administrator\\Desktop\\123.jpg");
		/*SkBitmap bitmap;
		SkImageDecoder::DecodeFile("C:\\Users\\Administrator\\Desktop\\123.jpg", &bitmap);
		SkPaint paint1(paint);
		SkScalar matrixScale = paint1.setupForAsPaths();
