...
namespace SOUI
{
		...
		,m_xferMode(kSrcOver_Mode)
	{
		...
		m_emojiBmp = new SBitmap_Skia(pRenderFactory);
		m_emojiBmp->LoadFromFile(L"C:\\Users\\harlinhuang\\Desktop\\smiles_01_01.png");
		SBitmap_Skia * temp = new SBitmap_Skia(pRenderFactory);
		//m_emojiBmp->Scale(&temp, 2, FilterLevel::kNone_FilterLevel);
...
// 				}
	...
	HRESULT SRenderTarget_Skia::DrawText( LPCTSTR pszText,int cchLen,LPRECT pRc,UINT uFormat)
	{
        ...
        SkRect skrc=toSkRect(pRc);
        skrc.offset(m_ptOrg);
		m_SkCanvas->m_emojiBmp = m_emojiBmp->GetSkBitmap();
		txtPaint.m_emojiBmp = m_emojiBmp->GetSkBitmap();
...
//         STRACE(L"path delete; objects = %d",--s_cPath);
	...
	//{
	//	m_bitmap = new SBitmap_Skia(m_renderfactory);
